...
*/			{
	...
	protected static function createSelector(array $fields)
	{
		...
		foreach ($entityTypeNames as $entityTypeName)
		{
			...
			foreach ($fieldNameMap as $seedFieldName => $targetFieldName)
			{
				if (is_numeric($seedFieldName))
				{
					$seedFieldName = $targetFieldName;
				}

				// skip if target field value filled
				if (isset($targetFields[$targetFieldName]) && $targetFields[$targetFieldName])
				{
					continue;
				}

				// skip if seed field not exists
				if (!isset($seedFields[$seedFieldName]) || !$seedFields[$seedFieldName])
				{
					continue;
				}

				$targetFields[$targetFieldName] = $seedFields[$seedFieldName];
			}
		}
