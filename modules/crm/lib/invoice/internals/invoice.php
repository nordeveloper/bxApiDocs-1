...
class InvoiceTable extends Main\Entity\DataManager
{
	...
	public static function getMap()
	{
		...
		return array(
			...
			new Main\Entity\ReferenceField(
				'BASKET',
				'Bitrix\Crm\Invoice\Internals\Basket',
				array(
			...
			new Main\Entity\ReferenceField(
				'PAYMENT',
				'Bitrix\Crm\Invoice\Internals\Payment',
				array(
			...
			new Main\Entity\ReferenceField(
				'SHIPMENT',
				'Bitrix\Crm\Invoice\Internals\Shipment',
				array(
					'=ref.ORDER_ID' => 'this.ID',
				)
			),
			...
			new Main\Entity\ReferenceField(
				'PROPERTY',
				'Bitrix\Crm\Invoice\Internals\OrderPropsValue',
				array(
					'=ref.ORDER_ID' => 'this.ID',
				),
				array('join_type' => 'INNER')
...
//			),
			...
			new Main\Entity\ExpressionField(
				'BY_RECOMMENDATION',
				"(SELECT (CASE WHEN MAX(BR.RECOMMENDATION) IS NULL OR MAX(BR.RECOMMENDATION) = '' THEN 'N' ELSE 'Y' END) FROM b_crm_invoice_basket BR WHERE BR.ORDER_ID=%s GROUP BY BR.ORDER_ID)",
				array('ID')
			)
		);
	}
