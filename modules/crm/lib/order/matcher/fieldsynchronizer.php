...
class FieldSynchronizer
{
	...
	private static function updateProperties($items)
	{
		foreach ($items as $itemFields)
		{
			...
			$result = OrderPropsTable::update($propertyId, $itemFields);
			...
			if ($result->isSuccess())
			{
				...
				if (!empty($matchProperties) || $itemFields['UTIL'] === 'Y')
				{
					...
					{
						OrderPropsMatchTable::add(['SALE_PROP_ID' => $result->getId()] + $matchProperties);
					}
				}
			}
		}
	...
	public static function save($personTypeId, $fields, $relations = [])
	{
		...
		{
			static::addProperties($itemsToCreate);
		}

		if (!empty($itemsToUpdate))
		{
			static::updateProperties($itemsToUpdate);
		}
