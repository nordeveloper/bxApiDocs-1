...
class Instagram
{
	 ...
	 * @return array
	 */
	public static function getConnection($url = '')
	{
		$connection = [];

		$cache = Cache::createInstance();

		$uniqueString = static::$source.md5($url);

		if ($cache->initCache(self::AUTH_CACHE_TIME, $uniqueString, self::AUTH_CACHE_DIR))
		{
		...
		elseif ($cache->startDataCache())
		{
			...
			$infoOAuth = static::getProvider()->getAuthorizationInformation(urlencode($url));
			...
			else
			{
				...
				if (!empty($InvalidOauthAccessToken))
				{
					...
					$connection['ERRORS'][] = new Error(
						...
						Library::ERROR_CONNECTOR_MESSENGER_INVALID_OAUTH_ACCESS_TOKEN
					);
				}
				else
				{
