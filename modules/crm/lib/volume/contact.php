...
class Contact extends Crm\Volume\Base implements Crm\Volume\IVolumeClear, Crm\Volume\IVolumeClearActivity, Crm\Volume\IVolumeClearEvent, Crm\Volume\IVolumeUrl
{
	/** @var array */
	...
	public function measureEntity()
	{
		...
		if ($this->prepareFilter($query))
		{
			...
			$query
				...
				->registerRuntimeField('', new Entity\ExpressionField('ENTITY_SIZE', 'COUNT(%s) * '.$avgContactTableRowLength, 'ID'))
	...
	public function measureActivity($additionActivityFilter = array())
	{
		...
		if ($this->prepareFilter($query))
		{
			...
			$querySql = $this->prepareActivityQuery(array(
				'DATE_CREATE' => 'CONTACT_DATE_CREATE_SHORT',
			));
