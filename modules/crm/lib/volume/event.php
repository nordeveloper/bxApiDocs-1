...
class Event extends Crm\Volume\Base implements Crm\Volume\IVolumeClear, Crm\Volume\IVolumeClearFile, Crm\Volume\IVolumeUrl
{
	...
	public function getActionList($componentCommandAlias)
	{
		...
		//$dateMax = new Entity\ExpressionField('DATE_MAX', "DATE_FORMAT(MAX(%s), '%%Y-%%m-%%d')", 'DATE_CREATE');
	...
	public function prepareQuery($indicator = '')
	{
		...
		)
		{
			$companyRelation = new Entity\ReferenceField(
				...
				Entity\Query\Join::on('this.ENTITY_ID', 'ref.ID')->where('this.ENTITY_TYPE', \CCrmOwnerType::CompanyName),
		...
		)
		{
			$leadRelation = new Entity\ReferenceField(
				...
				Entity\Query\Join::on('this.ENTITY_ID', 'ref.ID')->where('this.ENTITY_TYPE', \CCrmOwnerType::ContactName),
		...
		else
		{
			$dealRelation = new Entity\ReferenceField(
				'DEAL',
				Crm\DealTable::class,
				Entity\Query\Join::on('this.ENTITY_ID', 'ref.ID')->where('this.ENTITY_TYPE', \CCrmOwnerType::DealName),
			...
			$leadRelation = new Entity\ReferenceField(
				'LEAD',
				Crm\LeadTable::class,
				Entity\Query\Join::on('this.ENTITY_ID', 'ref.ID')->where('this.ENTITY_TYPE', \CCrmOwnerType::LeadName),
			...
			$leadRelation = new Entity\ReferenceField(
				...
				Entity\Query\Join::on('this.ENTITY_ID', 'ref.ID')->where('this.ENTITY_TYPE', \CCrmOwnerType::QuoteName),
			...
			$leadRelation = new Entity\ReferenceField(
				...
				Entity\Query\Join::on('this.ENTITY_ID', 'ref.ID')->where('this.ENTITY_TYPE', \CCrmOwnerType::InvoiceName),
	...
	public function getEventFileMeasureQuery($indicator = '')
	{
		if (!class_exists('\\Bitrix\\Crm\\Volume\\EventFileReferenceTable'))
		{
			...
			\Bitrix\Main\Entity\Base::compileEntity(
				'EventFileReference',
				array(
		...
		$fileRef = new Entity\ReferenceField(
			'FILEREF',
			Crm\Volume\EventFileReferenceTable::class,
			Entity\Query\Join::on('this.EVENT_ID', 'ref.EVENT_ID'),
			array('join_type' => 'INNER')
		);
		...
		$file = new Entity\ReferenceField(
			'FILE',
			Main\FileTable::class,
			Entity\Query\Join::on('this.FILEREF.FILE_ID', 'ref.ID'),
			array('join_type' => 'INNER')
		);
		$query->registerRuntimeField('', $file);


		$fileSize = new Entity\ExpressionField('FILE_SIZE', 'SUM(IFNULL(%s, 0))', 'FILEREF.FILE.FILE_SIZE');
		$fileCount = new Entity\ExpressionField('FILE_COUNT', 'COUNT(%s)', 'FILEREF.FILE.ID');
		$query
	...
	public function measureEntity()
	{
		...
		if ($this->prepareFilter($query))
		{
			...
			$query
				...
				->registerRuntimeField('', new Entity\ExpressionField('ENTITY_COUNT', 'COUNT(DISTINCT %s)', 'EVENT_BY.ID'))
	...
	public function clearFiles()
	{
		...
		if ($this->prepareFilter($query))
		{
			$eventIds = new Entity\ExpressionField('EID', 'DISTINCT %s', 'EVENT_BY.ID');
