...
{
	if (
		$arCatalog['PRODUCT_TYPE'] == CCatalogSku::TYPE_FULL
		|| $arCatalog['PRODUCT_TYPE'] == CCatalogSku::TYPE_PRODUCT
	)
	...
	do
	{
				...
				if (!empty($offers))
				{
					...
					foreach (array_keys($offers) as $productId)
					{
						...
						foreach (array_keys($offers[$productId]) as $offerId)
						{
							...
							$productOffer['PRODUCT_VAT_ID'] = (int)$productOffer['PRODUCT_VAT_ID'];
							if ($productOffer['PRODUCT_VAT_ID'] == 0)
								$productOffer['PRODUCT_VAT_ID'] = $offersCatalog['VAT_ID'];
		...
		if (!empty($items))
		{
			foreach ($itemIdsList as $id)
			{
				...
				if ($row['PRODUCT_TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					$minOfferId = null;
					$minOfferPrice = null;
					...
					foreach ($row['OFFERS'] as $offer)
					{
						...
						if ($vatExport && isset($vatList[$offer['PRODUCT_VAT_ID']]))
							$itemsContent .= "<vat>".$vatList[$offer['PRODUCT_VAT_ID']]."</vat>\n";
				...
				{
					$row['PRODUCT_VAT_ID'] = (int)$row['PRODUCT_VAT_ID'];
					if ($row['PRODUCT_VAT_ID'] == 0)
						$row['PRODUCT_VAT_ID'] = $arCatalog['VAT_ID'];
					...
					if ($vatExport && isset($vatList[$row['PRODUCT_VAT_ID']]))
						$itemsContent .= "<vat>".$vatList[$row['PRODUCT_VAT_ID']]."</vat>\n";
