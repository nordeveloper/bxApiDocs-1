...
class Common
{
	 ...
	 * @throws \Bitrix\Main\LoaderException
	 */
	public static function getEntityCard($entityType, $entityId)
	{
		$result = null;

		if(Loader::includeModule('im') && in_array($entityType, Array(Crm::ENTITY_LEAD, Crm::ENTITY_CONTACT, Crm::ENTITY_COMPANY, Crm::ENTITY_DEAL)))
		{
			$entityData = self::get($entityType, $entityId, true);
			...
			elseif($entityType == Crm::ENTITY_CONTACT)
			{
			...
			elseif($entityType == Crm::ENTITY_COMPANY || $entityType == Crm::ENTITY_DEAL)
			{
				if (isset($entityData['TITLE']))
				{
					$attach->AddLink(Array(
