	...
	public function Add($arFields, $arParams = array())
	{
		...
		if ($this->CheckFields($arFields, false, $effectiveUserId))
		{
			...
			if ($arParams['CLONE_DISK_FILE_ATTACHMENT'] === true || $arParams['CLONE_DISK_FILE_ATTACHMENT'] === 'Y')
			{
				...
				{
					$arFields['UF_TASK_WEBDAV_FILES'] = \Bitrix\Tasks\Integration\Disk::cloneFileAttachment(
						$arFields['UF_TASK_WEBDAV_FILES'],
						$effectiveUserId
					);
				}
			}
...
//							);
						...
						$DB->Query(
							...
							$DB->ForSql(\Bitrix\Tasks\Manager\Task::prepareSearchIndex($arFields)).
							'\' WHERE ID = '.
							$ID
						);
					}
		...
		if ($arTask = $rsTask->Fetch())
		{
			if ($this->CheckFields($arFields, $ID, $userID))
			{
				...
				if ($ufCheck)
				{
					...
					$arFields['SEARCH_INDEX'] = \Bitrix\Tasks\Manager\Task::prepareSearchIndex(
						array_merge($arTask, $arFields)
					);
...
//							);
						...
						{
							\Bitrix\Tasks\Integration\Forum\Task\Topic::updateTopicTitle(
								$arTask['FORUM_TOPIC_ID'],
								$arFields['TITLE']
							);
						}
	 ...
	 * @param $deadline
	 *
	 * @return Type\DateTime|int|static
	 */
	private static function getDeadlineMatchWorkTime($deadline)
	{
		$resultDeadline = Type\DateTime::createFromUserTimeGmt($deadline);
		...
		$resultDeadline = Type\DateTime::createFromTimestamp($resultDeadline - User::getTimeZoneOffsetCurrentUser());

		return $resultDeadline;
	}

	/**
	 * Occurs when user does not know anything about main task but is trying to change its subtask.
	 ...
	 * @return bool
	 */
	private static function fakeParentChange($oldParentId, $newParentId, $userId)
	{
		try
		{
			...
			if ($newParentId == false && $oldParentId)
			{
				...
				$parentTaskData = $parentTask->getData(array('SE_MEMBER'));

				$membersId = array();
				foreach ($parentTaskData['SE_MEMBER'] as $member)
				{
