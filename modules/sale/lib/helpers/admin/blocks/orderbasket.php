...
class OrderBasket
{
	...
	protected function getPropsForBasketItem($item, array $preparedData = array())
	{
		...
		if(is_array($params['PRODUCT_PROPS_VALUES']))
		{
			foreach($params['PRODUCT_PROPS_VALUES'] as $code => $value)
			{
				if(!isset($itemPropsCodes[$code]))
				{
					unset($params['PRODUCT_PROPS_VALUES'][$code]);
				}
			}
		}
